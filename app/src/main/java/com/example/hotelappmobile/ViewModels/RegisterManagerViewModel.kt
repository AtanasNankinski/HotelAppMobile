...
class RegisterManagerViewModel : ViewModel() {
    ...
    fun getHotelsByOwner(token: String, owner_id: Int){
        val hotelNamesTemp: MutableList<String> = mutableListOf()

        viewModelScope.launch {
            val response = hotelRepository.getHotelsByOwner("Bearer $token", owner_id)
    ...
    fun registerManager(body: HotelUser, token: String){
        viewModelScope.launch {
            val response = authRepository.registerManager("Bearer $token", body)
            if (response.isSuccessful){
                errorMessage.value = ""
